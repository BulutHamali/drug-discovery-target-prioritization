...
function Overview({ nav }: { nav: (v: View) => void }) { return <>
  ...
  <section className="panel architecture-panel"><div><span className="eyebrow">AWS-NATIVE COMPUTE PATH</span><h3>From population data to ranked targets.</h3><p>Scalable genomic processing runs in AWS; this results explorer is the lightweight presentation layer.</p></div><div className="architecture-flow"><span><b>1000 Genomes</b><small>S3 open data</small></span><i>→</i><span><b>AWS Batch</b><small>Spot compute</small></span><i>→</i><span><b>Feature matrix</b><small>Parquet</small></span><i>→</i><span><b>ML ranking</b><small>GroupKFold</small></span><i>→</i><span className="flow-result"><b>Results</b><small>Vercel explorer</small></span></div></section>
...
function Targets() {
  ...
  return <><section className="intro"><span className="eyebrow">TARGET EXPLORER</span><h2>Move from a score to a biological question.</h2><p>Search the out-of-sample ranking, separate known clinical labels from unlabeled genes, and inspect the provenance of each row. Scores are prioritization evidence—not druggability claims.</p></section>
    {!payload ? <><div className="target-toolbar target-toolbar-disabled"><label><span>Search gene symbol</span><input disabled placeholder="Waiting for ranking artifact" /></label><label><span>Clinical label</span><select disabled><option>All genes</option></select></label><div className="target-count"><strong>—</strong><span>targets available</span></div></div><div className="panel empty-targets">{error ? <><div className="empty-icon">≋</div><h3>Ranked predictions are not bundled yet</h3><p>Search and filtering are implemented, but the generated model output is not present in this repository.</p><p>Run <code>python3 ml/train_eval.py --feature-set biology_only</code><br />then <code>cd frontend && npm run export-targets</code>.</p></> : <><div className="empty-icon">…</div><h3>Loading ranked predictions</h3><p>Checking for the generated target artifact.</p></>}<div className="target-contract"><span>Expected fields</span><code>symbol · score · label · rank · fold_idx</code></div></div></> : <>
